...
big_predictor_win <- rbind(bo_2010, bo_2012, bo_2014, bo_2016, bo_2018)
...
boq_2011 <- merge(toq_2011, twenty11, by.x = "Winner", by.y = "X.1")
boq_2013 <- merge(toq_2013, twenty13, by.x = "Winner", by.y = "X.1")
boq_2015 <- merge(toq_2015, twenty15, by.x = "Winner", by.y = "X.1")
boq_2017 <- merge(toq_2017, twenty17, by.x = "Winner", by.y = "X.1")
boq_2019 <- merge(toq_2019, twenty19, by.x = "Winner", by.y = "X.1")

big_predictor_lose <- rbind(boq_2011, boq_2013, boq_2015, boq_2017, boq_2019)
...
## Need to swap DV3 with DV1

finally <- rbind(big_predictor_win, big_predictor_lose)
finally <- finally[,-c(27,28,31,32,35,36,71,72,75,76)] ## data not completely good but tourney starts manana
finally2 <- finally[,c(6,14:19,25:68)]
dat2 <- data.frame(lapply(finally2, function(x) as.numeric(as.character(x))))
...
logit.CV <- train(DV1 ~ ., data=user, 
                  method = 'glmnet',
                  trControl = trControl2,
                  family = 'binomial' )


user$DV1 <- as.factor(as.character(user$DV1))
logit.CV2 <- train(DV1 ~ ., data=user, 
                  method = 'glmnet',
                  trControl = trControl2,
                  family = 'binomial' )

predictions2 <- predict(logit.CV2, validate)
write.csv(predictions2, "winnerLOGISTIC.csv")
